<template>
	<div>
		<div class="ptlogin">
			<p>平台/机器人注册</p>
			<div class="froms">
				<label style="margin-left: 23%;"><input  type="radio" name="sex" v-model="radios" value="1"/>平台 </label> 
				<label><input  type="radio" name="sex"  v-model="radios" value="2"/>机器人 </label>
				<el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
				  <el-form-item label="平台/机器人名称" prop="pass" style="white-space:nowrap;">
				    <el-input  placeholder="公司名称+编号 填写样例: 万维-01" v-model="ruleForm.pass" autocomplete="off" style="padding-left:20px;"></el-input>
				  </el-form-item>
				  <!--<el-form-item label="姓名" prop="checkPass">
				    <el-input  placeholder="名称 填写样例: 张凯" v-model="ruleForm.checkPass" autocomplete="off" style="padding-left:20px;"></el-input>
				  </el-form-item>-->
				  <el-form-item label="电话" prop="age">
				    <el-input v-model.number="ruleForm.age" placeholder="电话 填写样例: 15512345678" style="padding-left:20px;"></el-input>
				  </el-form-item>
				  <!--<el-form-item label="管理员邮箱" prop="mail">
				    <el-input @input="intchange($event)"  autocomplete="off"   v-model.number="ruleForm.mail" placeholder="邮箱 填写样例: username@mail.com" style="padding-left:20px;"></el-input>
				  </el-form-item>-->
				  ...
				  <el-form-item label="一级部门" prop="onebm" style="clear:both;padding-top:10px;">
	...
	export default{
		...
		data() {
		    ...
		    var validatePass = (rule, value, callback) => {
		        if (value === '') {
		          callback(new Error('名称不能为空|填写样例：万维-01(公司名称+编号)'));
		        }
		    };
		    /*var validatePass2 = (rule, value, callback) => {
		        if (value === '') {
		          callback(new Error('管理员姓名不能为空|填写样例：张凯'));
		        }
		    };*/
	      ...
	      submitForm(formName) {
	      	...
	      	if (this.ruleForm.pass!='' && this.ruleForm.checkPass!='' && this.ruleForm.age!='' && this.ruleForm.mail!=''  && this.ruleForm.onebm!=''  && this.ruleForm.twobm!='') {
	            ...
	            var obj =JSON.stringify({
		          	type:this.radios,//1：平台注册；2：机器人注册；3：管理员注册
		          	name:this.ruleForm.pass,//平台名称/机器人名称名称
		          	username:this.ruleForm.checkPass,//管理员名称
	            ...
	            this.$axios.post(baseurl+'/robot/secure/common/register',obj).then((res)=>{
	            	...
	            	/*if(res.data.message=='已通过注册认证，请勿重复认证'){
	            		...
	            		//弹框
		            	if(this.radios==1){
		            		this.$alert('<strong>平台注册成功</strong>', '注册', {
					          dangerouslyUseHTMLString: true
					        }).then(() => {
					        	this.$router.push({path:'/loginindex'})
					        })
		            	}else{
		            		this.$alert('<strong>机器注册成功</strong>', '注册', {
					          dangerouslyUseHTMLString: true
					        }).then(() => {
					        	this.$router.push({path:'/loginindex'})
					        })
